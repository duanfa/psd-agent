...
设计任务是以品牌和商品为上下文的一次生成请求。当前 MVP 阶段聚焦商品详情页场景，贯穿任务资料解析、页面规划、图片生成、布局生成和设计稿输出全过程。
...
- `design.md`：品牌设计规范说明文档
- `detail_layout.json`：商品详情页布局规范与模板
- `component_library.json`：品牌组件库定义
- `prompt_library.json`：不同任务的提示词模板

其中，`design.md` 用于沉淀品牌层面的通用设计约束，`detail_layout.json` 用于沉淀商品详情页场景下的模块结构、信息层级、组件布局和版式规律。以上结果统一存入品牌知识库，作为后续商品详情页生成任务的基础输入。
...
1. 任务理解：解析 `brief 线框表格` 中的文字信息、模块结构、排版信息和设计方向。
2. 页面规划：结合商品详情页布局规范，输出详情页模块结构与信息顺序。
3. 图片生成：结合素材图补充卖点图、氛围图和功能图等页面所需视觉内容。
4. 版式设计：根据品牌设计规范和商品详情页布局规范完成页面布局。
...
```mermaid
flowchart TD
    A[上传 brief 线框表格与素材图] --> B[任务理解 Agent]
    B --> C[生成 Task Brief]
    C --> D[页面规划 Agent]
    D --> E[生成页面结构与模块顺序]
    E --> F[图片生成 Agent]
...
- 当前 MVP 阶段训练产物聚焦为品牌设计规范和商品详情页布局规范，不包含首页和二级页相关规则产物。
...
- 上传品牌资料后，系统可自动生成 `design.md` 和 `detail_layout.json`。
- 系统可输出可被后续流程消费的结构化规则文件。
- 品牌规则结果可查看、可管理、可更新版本。

### 11.2 商品生成验收

- 上传 `brief 线框表格` 和素材图后，系统可自动生成商品详情页方案。
- 在标准数据输入下，5 分钟内完成详情页生成。
- 生成结果支持预览、编辑和导出 Figma，并兼容 PSD 设计稿输出能力。
- 设计稿在风格、配色、字体和模块结构上符合品牌设计规范与商品详情页布局规范。
